...
#        plots.extend(gps_plots)
    ...
    def add_plot(gen_plot_cb, plot_name=None):
        """
        add a plot to the plots list. It shows a caches image if found
        (non-interactive), otherwise uses a bokeh plot.
        ...
        """
        nonlocal need_cache_update, plots, log_id
...
#            return None
        ...
        if os.path.isfile(image_name):
            # show the cached file with a button to request the interactive plot
            plot_index = len(plots)
            load_plot_button = Button(label="Load interactive Plot", width=150)
            def load_plot_button_clicked(index):
                start_time = timer()
                main_layout = curdoc().get_model_by_name('main_layout')
                plot = gen_plot_cb()
                if plot is not None:
                    plot.set_name(plot_name)
                    main_layout.children[index] = plot.bokeh_plot
                if debug_verbose_output():
                    print_timing("dynamic plot loading", start_time)

            load_plot_button.on_click(lambda i=plot_index: load_plot_button_clicked(i))
            plots.append(row(Div(
                text='<img src="%s">' % image_name , width=int(plot_width)),
                Spacer(width=20), load_plot_button))
        else:
            # show the bokeh plot
            plot = gen_plot_cb()
            if plot is not None:
    ...
    def plot_raw_angular_speed():
        ...
        data_plot.add_graph([
            ...
            lambda data: ('gyro_rad[2]', np.rad2deg(data['gyro_rad[2]']))],
                            colors3, ['X', 'Y', 'Z'])
        return data_plot.finalize()
    add_plot(plot_raw_angular_speed)
    ...
    def plot_magnetic_field_strength():
        ...
        data_plot.add_graph(['magnetometer_ga[0]', 'magnetometer_ga[1]',
                            ...
                            ['X', 'Y', 'Z'])
        return data_plot.finalize()
    add_plot(plot_magnetic_field_strength)
    ...
    def plot_thrust_and_magnetic_field():
        ...
        data_plot.add_graph([lambda data: ('thrust', data['control[3]'])],
                            colors2[1:2], ['Thrust'])
        return data_plot.finalize()
    add_plot(plot_thrust_and_magnetic_field)
    ...
    def plot_estimator_watchdog():
        ...
        return data_plot.finalize()
    add_plot(plot_estimator_watchdog)
...
#    start_time = timer()
#    print(export(data_plot.bokeh_plot, 'sampling.png'))
#    print_timing("png export sampling", start_time)
    ...
    if need_cache_update:
        if debug_verbose_output():
            print('Requesting plot uploads')
        curdoc().template_variables['upload_renderings'] = True

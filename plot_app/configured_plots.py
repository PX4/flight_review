...
# TODO: don't generate plot for extended...
        plot = gen_plot_cb()
        if plot is not None:
            plot.set_name(plot_name)
            if extended:
                extended_plots.append(plot)
            else:
                plots.append(plot)
    ...
    # load extended plots button
    load_plots_button = Button(label="Load extended Plots", width=170)
    def load_plots_button_clicked():
        # TODO: hide button
        main_layout = curdoc().get_model_by_name('main_layout')
        main_layout.children.extend([p.bokeh_plot for p in extended_plots])

...
class DownloadHandler(TornadoRequestHandlerBase):
    ...
    def get(self, *args, **kwargs):
        ...
        def get_original_filename(default_value, new_file_suffix):
            ...
            try:
                con = sqlite3.connect(
                    get_db_filename(), detect_types=sqlite3.PARSE_DECLTYPES
                )
                cur = con.cursor()
                cur.execute(
                    "select OriginalFilename " "from Logs where Id = ?", [log_id]
                )
        ...
        if download_type == "1":  # download the parameters
            ...
            self.set_header("Content-Disposition", "inline; filename=params.params")
            ...
            self.write("#\n")
            self.write("# Vehicle-Id Component-Id Name Value Type\n")
            ...
            for param_key in param_keys:
                ...
                self.write(
                    f"{vehicle_id}\t{component_id}\t{param_key}\t{param_value:.15f}\t{param_type}\n"
                )

        elif download_type == "2":  # download the parameters
            ulog = load_ulog_file(log_file_name)
            param_keys = sorted(ulog.initial_parameters.keys())
            ...
            self.write("# Onboard parameters for Vehicle {}\n".format(vehicle_id))
            self.write("#\n")
            self.write("# Stack: PX4 Pro\n")
            self.write("# Vehicle: Multi-Rotor\n")
            self.write("# Version: {}\n".format(version))
            self.write("# Git Revision: {}\n".format(git_revision))
            self.write("#\n")
            self.write("# Vehicle-Id Component-Id Name Value Type\n")
            ...
            if ulog.has_default_parameters:
                ...
                for param_key in param_keys:
                    try:
                        ...
                        if not is_default:
                            self.write(
                                f"{param_key}\t{param_value:.15f}\t9\n"
                            )  # Write the parameter in .params format
                    except:
                        pass
                    ...
                    try:
                        ...
                        if param_key in default_params:
                            ...
                            if default_param["type"] == "FLOAT":
                                ...
                                is_default = int(default_param["default"]) == int(
                                    param_value
                                )

                        if not is_default:
                            self.write(
                                f"{param_key}\t{param_value}\t9\n"
                            )  # Write the parameter in .params format
                    except:
                        pass

        elif download_type == "4":  # download the kml file
            ...
            if not os.path.exists(kml_file_name):
                ...
                def kml_colors(flight_mode):
                    ...
                    if flight_mode not in flight_modes_table:
                        flight_mode = 0

                    color_str = flight_modes_table[flight_mode][1][
                        1:
                    ]  # color in form 'ff00aa'
                    ...
                    return (
                        "ff" + color_str[4:6] + color_str[2:4] + color_str[0:2]
                    )  # KML uses aabbggrr
                ...
                try:
                    ...
                    print(
                        "Error creating KML file", sys.exc_info()[0], sys.exc_info()[1]
                    )
            ...
            self.set_header(
                "Content-Disposition", "attachment; filename=" + kml_dl_file_name
            )
        ...
        else:  # download the log file (default behavior)
            self.set_header("Content-Type", "application/octet-stream")
            self.set_header("Content-Description", "File Transfer")
            self.set_header(
                "Content-Disposition",
                "attachment; filename={}".format(os.path.basename(log_file_name)),
            )
